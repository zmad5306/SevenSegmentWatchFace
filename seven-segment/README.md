...
![preview](src/main/res/drawable-nodpi/preview.png)
...
## Layout
`tools/generate_watchface.py` is the source of truth. It writes these files (paths relative to `seven-segment/`):

| File                                               | Contents                   |
|----------------------------------------------------|----------------------------|
| `src/main/res/drawable-nodpi/seg_time_*.png`       | native clock bitmap glyphs |
| `src/main/res/raw/watchface.xml`                   | the WFF scene              |
| `src/main/res/values/strings.xml`                  | editor labels              |
| `src/main/res/drawable-nodpi/preview.png`          | watch face picker preview  |
| `src/main/res/drawable/ic_launcher_foreground.xml` | app icon artwork           |
| `src/main/res/mipmap-anydpi/ic_launcher.xml`       | adaptive app icon          |

Don't edit those files by hand. Change the geometry, palette, or options in the script, then regenerate:

```bash
python seven-segment/tools/generate_watchface.py
```
...
```bash
adb install -r seven-segment/build/outputs/apk/debug/seven-segment-debug.apk
```
...
```bash
python -m unittest discover -s seven-segment/tools -p "test_*.py"
./gradlew :seven-segment:assembleDebug :seven-segment:assembleRelease :seven-segment:lintDebug
```
